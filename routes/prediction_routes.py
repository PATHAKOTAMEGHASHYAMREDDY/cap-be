...
def predict():
    ...
    try:
        ...
        prediction_result = image_prediction(image_array)
        ...
        raw_probs = prediction_result.get('raw_probabilities', [[0, 0, 0]])[0]
        ...
        response_data = {
            ...
            'prediction': {
                ...
                'confidence': {
                    'control': round(float(raw_probs[0]) * 100, 2),  # Convert to percentage (0-100)
                    'alzheimer': round(float(raw_probs[1]) * 100, 2),  # Convert to percentage (0-100)
                    'parkinson': round(float(raw_probs[2]) * 100, 2)   # Convert to percentage (0-100)
                },
                'primary_confidence': round(prediction_result['confidence'] * 100, 2)  # Convert to percentage (0-100)
            },
            'metadata': {
                'filename': filename,
        ...
        logger.info(f"Prediction made for user {current_user_id}: {prediction_result['prediction']}")
...
def generate_report():
    ...
    try:
        ...
        patient_data = [
            ...
            ['User Type:', user.user_type.title() if user else "Unknown"],
        ...
        # Primary diagnosis
        diagnosis_color = colors.HexColor('#10b981') if results['prediction'] == 'CONTROL' else colors.HexColor('#f59e0b')
        ...
        confidence_data = [
            ['Condition', 'Confidence Level', 'Status'],
            ['Normal/Control', f"{results['confidence']['control']:.1f}%", 
             'Primary' if results['prediction'] == 'CONTROL' else 'Secondary'],
            ['Alzheimer\'s Disease', f"{results['confidence']['alzheimer']:.1f}%", 
             'Primary' if results['prediction'] == 'AD' else 'Secondary'],
            ['Parkinson\'s Disease', f"{results['confidence']['parkinson']:.1f}%", 
             'Primary' if results['prediction'] == 'PD' else 'Secondary']
        ]
        
        confidence_table = Table(confidence_data, colWidths=[2.5*inch, 1.5*inch, 1.5*inch])
        ...
        if results.get('metadata'):
            ...
            tech_data = [
                ['Analysis File:', results['metadata'].get('filename', 'Unknown')],
                ['Model Version:', results['metadata'].get('model_version', 'Unknown')],
                ['Processing Time:', results['metadata'].get('processing_time', 'N/A')],
                ['Analysis ID:', str(current_user_id)[:8] + '...' if current_user_id else 'Unknown']
            ]
        ...
        disclaimer_text = results.get('disclaimer', 
            'This AI analysis is for informational purposes only and should not replace professional medical diagnosis. Please consult with a qualified healthcare provider for proper medical evaluation.')
        ...
        story.append(Paragraph("Generated by Medical AI Analysis System", footer_style))
        ...
        # Save PDF to uploads folder for download
        ...
        logger.info(f"PDF report generated for user {current_user_id}: {report_filename}")
        
        return jsonify({
            'success': True,
            'message': 'Report generated successfully',
            'download_url': f'/static/uploads/{report_filename}',
